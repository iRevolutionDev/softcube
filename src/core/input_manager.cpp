...
int InputManager::register_key_callback(int keyCode, std::function<void(int, KeyState)> callback) {
    if (keyCode < 0 || keyCode >= 512) {
        SC_LOG_ERROR("Invalid key code: {} for callback registration", keyCode);
        return -1;
    }
    ...
    SC_LOG_DEBUG("Registered callback {} for key {}", callback_id, keyCode);
    return callback_id;
}
...
void InputManager::unregister_key_callback(int id) {
    ...
    if (it != key_callbacks.end()) {
        SC_LOG_DEBUG("Unregistered callback {} for key {}", id, it->second.key_code);
        key_callbacks.erase(it);
    } else {
        SC_LOG_WARN("Attempted to unregister non-existent callback: {}", id);
    }
}
...
std::string InputManager::get_key_name(int keyCode) const {
